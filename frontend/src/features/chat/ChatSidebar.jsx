...
export default function ChatSidebar() {
  ...
  const { conversations, activeConversationId, conversationsStatus, search } = useSelector(
    (state) => state.chat
  );
  ...
  return (
    <aside className="hidden h-screen min-h-0 flex-col border-r border-line bg-white/78 backdrop-blur-xl lg:flex">
      <div className="flex h-16 shrink-0 items-center gap-3 border-b border-line px-5">
        ...
        <div>
          <p className="text-sm font-black text-ink">Nova</p>
          <p className="text-xs text-slate-500">AI conversation hub</p>
        </div>
      </div>
      ...
      <div className="space-y-3 p-4">
        ...
        <label className="relative block">
          ...
          <input
            ...
            className="focus-ring h-11 w-full rounded-lg border border-line bg-white pl-10 pr-3 text-sm"
            ...
            return (
              ...
              >
                {editingId === conversation._id ? (
                  <form onSubmit={(event) => submitRename(event, conversation._id)}>
                    <input
                      autoFocus
                      value={title}
                      onChange={(event) => setTitle(event.target.value)}
                      onBlur={() => setEditingId(null)}
                      className="focus-ring h-9 w-full rounded-lg border border-line px-3 text-sm"
                    />
                  </form>
                ) : (
                  <button
                ...
                <div className="mt-3 flex items-center justify-between">
                  ...
                  >
                    <button
                      ...
                      className="grid h-8 w-8 place-items-center rounded-lg text-slate-500 hover:bg-white hover:text-brand-700"
                    ...
                    <button
                      ...
                      className="grid h-8 w-8 place-items-center rounded-lg text-slate-500 hover:bg-red-50 hover:text-coral"
        ...
        <div className="rounded-lg border border-line bg-white p-3 shadow-sm">
          <div className="flex items-center gap-3">
            ...
            <div className="min-w-0 flex-1">
              <p className="truncate text-sm font-bold text-ink">{user?.name || "Nova User"}</p>
              <p className="truncate text-xs text-slate-500">{user?.email || "Workspace profile"}</p>
            </div>
          </div>
          <button
            type="button"
            onClick={() => dispatch(logout())}
            className="mt-3 inline-flex h-10 w-full items-center justify-center gap-2 rounded-lg border border-line bg-white text-sm font-bold text-slate-700 transition hover:border-coral hover:text-coral"
          >
